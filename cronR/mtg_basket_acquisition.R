...
for(i in distinct_basket_dates$date){
    ...
    mybq <- bq_table(project = "gaeas-cradle", dataset = "mtg_basket", table = paste(gsub("-","_",expanded_baskets_tbl %>% select(date)%>%  mutate(date = as.Date(anytime(date))) %>%distinct() %>% .[[1]]) ,"_mtg_basket",sep=""))
