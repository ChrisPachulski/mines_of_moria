...
#install.packages("chronR")
install.packages('chronR', repos = "http://cran.us.r-project.org")
#
...
for(i in 1:683){
  ...
  prices <- as.data.frame(prices)
  CK_NF_MKT <- data_frame(
    X1 <- paste(CK_name,CK_set,CK_rarity," ",sep =""),
    X2 <- CK_name,
    X3 <- CK_set,
    X4 <- CK_rarity,
    X5 <- prices)
...
write.csv(Final_Export, file=csvFileName, row.names = FALSE) 
...
#Funny Money Report####
summary(CK_NF_MKT$`Primary Key`)
summary(TCG_Vendor$Primary_Key)

CK_NF_MKT$TCG_Price <- TCG_Vendor$MKT_EST[match(CK_NF_MKT$`Primary Key`,TCG_Vendor$Primary_Key)]
CK_NF_MKT$TCG_Price <- as.numeric(CK_NF_MKT$TCG_Price)
CK_NF_MKT$CK_TCG_PCT_DIFF <- CK_NF_MKT$CK_Price/CK_NF_MKT$TCG_Price
CK_NF_MKT
