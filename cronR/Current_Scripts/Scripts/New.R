...
#
install.packages('shinyFiles', repos = "http://cran.us.r-project.org")
#
...
for(i in 1:35){
  ...
  html <- read_html(url)  #html <- read_html(url) html <- read_html("scrapedpage.html")
...
View(Final_Export)
Final_Export <- Final_Export[c(1,2,4,5,6,12,10,9,11,13,8,7,15,14,16)]
names(Final_Export)<- c("Key","Card","Set","Rarity","F/NF","BL_QTY","BL","MKT","Arb","Sellers","TCG_Rank","CK_Rank","CK_ADJ_Rank","OVR_Rank","%_of_Market")
#Final_Export$BL_Value <- test$BL_Value[match(Final_Export$CK_Key,Ranking_Export$CK_Key)]
Final_Export <- Final_Export[order(-Final_Export$`%_of_Market`,-Final_Export$Arb,Final_Export$OVR_Rank),]
...
names(CK_Price_Comparison) <- c("Primary_Key","Card_Name","Set","Rarity","TCG_Price","CK_Price")
...
CK_Price_Comparison$CK_BL <- Final_Export$BL[match(CK_Price_Comparison$Primary_Key,Final_Export$Key)]
...
CK_Price_Comparison$TCG_Vendors <- Final_Export$Sellers[match(CK_Price_Comparison$Primary_Key,Final_Export$Key)]
CK_Price_Comparison$BL_Desired_Amt <- Final_Export$BL_QTY[match(CK_Price_Comparison$Primary_Key,Final_Export$Key)]
