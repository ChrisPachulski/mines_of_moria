...
for(i in 1:1000){
  ...
  #nrow(bitp)
  bits <- NULL
  bit <- if(nrow(bit)<10){rbind(bit, NA)}else{(bit)}
  bits <- if(nrow(bit) == 10) {as.list(bit)}else{as.list(bitp)}
  ...
  bit_set <- suppressWarnings(data.frame(do.call('rbind', strsplit(as.character(bit_set),'(',fixed=TRUE))))
  bit_set <- trimws(bit_set$X1)
...
Pricing_Export <- Pricing[c(2,8,9,10,11,12,14,15,6,16,3,17,4)]
...
Final_Export <- Final_Export[c(1,2,4,5,6,13,11,10,12,14,9,7,15,17)]
...
library(readxl)
Names <- read_csv("~/Reports/All_Cards_TBD.csv")
...
#View(Vendor_Growth)
Consistent_Vendors <- Consistent_Vendors[c(-5)]
...
#Export Top Movers - Buylist Backed - Reports####
...
#View(Month_Ago)
Goldilocks_TCG$Month_CK_Demand <- Month_Ago$CK_Rank[match(Goldilocks_TCG$Unique_Keys, Month_Ago$Key)]
MTH_BL_Import <- paste("/home/cujo253/Reports/TCG Vendor/",MTH,"_TCG.csv", sep = "")
Month_Ago <-read_csv(MTH_BL_Import, col_types = cols(MKT_EST = col_number(), Rank = col_number(), `Vendor Listings` = col_number()))
...
Growing <- list(as.data.frame(Consistent_Sellers), as.data.frame(Consistent_Vendors), as.data.frame(CB_Short))
sheets_create(
  paste(currentDate,"_Growth_Reports",sep=""),
  sheets = Growing
)
...
library(mailR)
# 
Practice_List <- c("cjpach@mac.com")
Practice_List <- as.data.frame(Practice_List)
# 
for (recipient in Practice_List$Practice_List){
 ...
 Pet Moose, Snuggle Ruby, hug Dana, and rest easy knowing your Scrape ran without you needing to be there.
