...
temp <- list.files(pattern="*.csv")
Number_Of_Files <- length(temp) - 1
start_date <- currentDate #- 3
Title_Date <- gsub("\\-","\\_",start_date)
combined_file <- NULL

for (i in 1:3){
  tmp <- tryCatch({read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Vendor/",start_date,"_Vendor_Times_Series.csv",sep=""), 
    ...
    error = function(e){read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Vendor/",start_date,"_Vendor_Times_Series.csv",sep=""), 
                                 col_types = cols(`F/NF` = col_character())) %>% rename(c(`F/NF` = "Foil")) %>% 
        mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = start_date) %>% select(uuid,everything())})
...
for (i in 1:3){
  tmp <- tryCatch({read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Buylist/",start_date,"_Times_Series.csv",sep=""), 
                            col_types = cols(`F/NF` = col_character())) %>% rename(c("Foil" = `F/NF`)) %>% 
      mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = start_date) %>% select(uuid,everything())},
      error = function(e){read_csv(paste("/home/cujo253/Metrics/Time_Series_Results/Buylist/",start_date,"_Times_Series.csv",sep=""), 
                                   col_types = cols(`F/NF` = col_character())) %>% rename(c(`F/NF` = "Foil")) %>% 
          mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = start_date) %>% select(uuid,everything())})
