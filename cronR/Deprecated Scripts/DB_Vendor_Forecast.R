...
Final_Compilation <- range_read(ss,"All_Cards") %>% filter(Todays_BL != 3)
Sets <- read.csv("/home/cujo253/Essential_Referential_CSVS/Sets.csv",stringsAsFactors = TRUE)
...
Final_Compilation_Export <- tryCatch({Final_Compilation %>% rename(c(`F/NF` = "Foil")) %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = Sys.Date())}, error = function(e){Final_Compilation %>% rename(c("Foil" = `F/NF`)) %>% mutate(uuid = Updated_Tracking_Keys$uuid[match(Key,Updated_Tracking_Keys$Key)], Date = Sys.Date())})
#Final_Compilation_Export <- Final_Compilation_Export %>% select(uuid,everything())
...
bq_table_upload(x=mybq, values = Final_Compilation_Export, fields=as_bq_fields(Final_Compilation_Export),nskip = 1, source_format = "CSV",create_disposition = "CREATE_IF_NEEDED", write_disposition = "WRITE_TRUNCATE")
...
BL_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) 
...
Vendor_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1)
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.buylist_time_series.",gsub("-","_",DelayDate_1),"_BL_FORECAST` a ",sep = "")
BL <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% distinct()
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.vendor_time_series.",gsub("-","_",DelayDate_1),"_VENDOR_FORECAST` a ",sep = "")
Vendor <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% distinct()
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.buylist_time_series.",gsub("-","_",DelayDate_1),"_BL_FORECAST` a ",sep = "")
BL_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% distinct()
...
statement <- paste("SELECT *   ","FROM `gaeas-cradle.vendor_time_series.",gsub("-","_",DelayDate_1),"_VENDOR_FORECAST` a ",sep = "")
Vendor_Delay_Fourteen <- dbSendQuery(con, statement = statement) %>% dbFetch(n = -1) %>% distinct()
