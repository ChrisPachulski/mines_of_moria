...
statement <- paste(
    ...
    'NOT regexp_contains(a.set, r"Alpha|Beta|Guild Kit|^Portal|^Unlimited|Arabian|^Starter|Duel Deck|^Legends|Antiquities") ',
    "Order By Date asc; ",
    sep = ""
)
...
# full_tbl %>% filter(grepl("Academy RectorUrza's DestinyR",Key)) %>% view()
...
full_tbl = unique_card                                   %>%
    ...
    map(.f = function(df){
        df %>% 
            ...
            tk_augment_fourier(Date    ,.period = c(5,10,14,21,30,45,60))       %>%
            
            tk_augment_lags(BL         , .lags = c(3,5,7,14,32))   %>%
            
            tk_augment_lags(BL_QTY     , .lags = c(3,5,7,14,32))   %>%
            
            tk_augment_lags(MKT        , .lags = c(3,5,7,14,32))   %>%
            
            tk_augment_lags(Sellers    , .lags = c(3,5,7,14,32))   %>%
            
            tk_augment_lags(TCG_Rank   , .lags = c(3,5,7,14,32))   %>%
            
            tk_augment_lags(CK_ADJ_Rank, .lags = c(3,5,7,14,32))   %>%
...
rmse_to_keep = metrics %>% arrange(rmse) %>% mutate(rank = seq(nrow(.)), tier = ntile(rmse,10) )%>% filter(tier <= 1) %>% select(-tier)
...
boxplot_overview_tbl %>% group_by(Key) %>%
  slice(seq(3, nrow(.), by = 5))

grand_slam_tbl = boxplot_overview_tbl %>% group_by(Key) %>% dplyr::slice(seq(3,nrow(.),by = 5)) %>% ungroup() %>%
