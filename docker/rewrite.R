...
pacman::p_load(tidyverse,rvest,jsonlite,devtools,googlesheets4,googledrive,readr,dplyr,gargle,httr,bigrquery,RSelenium,janitor,googleAuthR)
...
for(i in 1:total) {
    ...
    tryCatch({
        ...
        if(ncol(Gold) > 4) {
            ...
            message("Insufficient columns encountered on: ", sets$MTG_Goldfish_Sets[i])
        }
    }, error = function(e) {
        message("Error encountered on URL: ", url, "\nError message: ", e$message)
    })
    Sys.sleep(.25)
}
...
#Low_Confidence_Report %>% filter(grepl('Double.*2022',Set))
...
print("Made it to Selenium Component")
remDr <- chrome("64.225.20.203")
print("Made it paste initial Selenium Component")
