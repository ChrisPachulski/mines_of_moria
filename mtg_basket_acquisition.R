...
suppressMessages(for(i in 1:nrow(tcg_ids_of_interest)) {
    ...
    for(b in 1:length(offsets)){
        ...
        if(test_value == 0){
            ...
            all_basket_raw = rbind(all_basket_raw,basket_line_items)
            
            
            
            all_basket_date_cleanse_tbl = all_basket_raw 
        }
    ...
    if(is.null(all_basket_date_cleanse_tbl)){next}
    write_csv(all_basket_date_cleanse_tbl,file = paste0(tcg_ids_of_interest[i,],".csv"))
    ...
    print(paste(i,nrow(tcg_ids_of_interest),scales::percent(i/nrow(tcg_ids_of_interest))))},
        error = function(e){print(paste0("Error with id: ",i))})
})
...
set_baskets = NULL
setwd("/Users/cujo253/mines_of_moria/Essential_Referential_CSVS/temp_tcg_cards")
set_baskets <-
    list.files(pattern = "*.csv") %>% 
...
days = 0
baskets_bucket = NULL
i = distinct_basket_dates$date[1]
...
for(i in distinct_basket_dates$date){
    ...
    mybq <- bq_table(project = "gaeas-cradle", dataset = "mtg_basket", table = paste(gsub("-","_",expanded_baskets_tbl %>% select(date)%>%  mutate(date = as.Date(date)) %>%distinct() %>% .[[1]]) ,"_mtg_basket",sep=""))
